...
\begin{itemize}
  ...
  \item Custos de transação: modelagem simplificada
\end{itemize}
\end{block}
  \column{0.47\textwidth}
\begin{block}{Código (70 min)}
\begin{itemize}
  \item \texttt{construir\_portfolio()} — top 20\%, shift(1)
...
\begin{itemize}
  \item Sharpe $> 1{,}0$: bom (geralmente difícil de alcançar)
  \item Sharpe $> 0{,}5$: aceitável para long-only
  \item MDD $< 20\%$: confortável para maioria dos mandatos
  \item Calmar $> 0{,}5$: risco/retorno razoável
\end{itemize}
...
\end{lstlisting}
  \column{0.47\textwidth}
\begin{block}{Custos de Transação}
\begin{itemize}
  \item Modelagem simplificada: $c = 0{,}1\%$ por operação
  \item Turnover mensal: $\sum_i |w_{i,t} - w_{i,t-1}|$
  \item Custo total: Turnover $\times c$
  \item Em média: momentum tem turnover de 30--50\% a.m.
\end{itemize}
\end{block}\vspace{.3em}
\begin{alertblock}{Hipóteses do backtest}
\begin{itemize}
  \item Execução ao preço de fechamento do mês
  \item Liquidez infinita (sem market impact)
  \item Sem short: long-only com equal-weight no top 20\%
...
\begin{itemize}
  \item \texttt{construir\_portfolio(sinal, ret, n\_pct=0.2)} $\to$ retornos da carteira
...
\begin{itemize}
  ...
  \item \textbf{Korajczyk, R. \& Sadka, R.} (2004). Are momentum profits robust to trading costs? \textit{Journal of Finance}, 59(3), 1039--1082.
